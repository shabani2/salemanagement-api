...
function assertCanAccessRegion(req, regionId) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const userRegionId = typeof user.region === 'string' ? user.region : (_a = user === null || user === void 0 ? void 0 : user.region) === null || _a === void 0 ? void 0 : _a._id;
        if (role === 'AdminRegion' && userRegionId && String(userRegionId) === String(regionId))
            return;
    ...
    try {
        ...
        if (match.region && !match.pointVente && !onlyRegion) {
            ...
            if (regionOID) {
                ...
                const pvIds = (yield model_1.PointVente.distinct('_id', { region: regionOID })).map((id) => toId(id)).filter(Boolean);
                // retirer la contrainte top-level `region` pour la remplacer par un $or
                const { region } = match, rest = __rest(match, ["region"]);
                const or = [{ region: regionOID }];
                if (pvIds.length)
                    or.push({ pointVente: { $in: pvIds } });
        ...
        const mouvements = yield populateAll(model_1.MouvementStock.find(match).sort({ [sortBy]: order }).skip(skip).limit(limit));
        sendPaged(res, { total, page, limit, mouvements });
        return;
    }
    catch (err) {
        const status = (_g = err === null || err === void 0 ? void 0 : err.status) !== null && _g !== void 0 ? _g : 500;
        res.status(status).json({ message: 'Erreur interne', error: (_h = err === null || err === void 0 ? void 0 : err.message) !== null && _h !== void 0 ? _h : err });
        return;
    }
});
...
const getMouvementStockByRegion = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const limit = Math.min(Math.max(parseInt(String((_b = req.query.limit) !== null && _b !== void 0 ? _b : '20'), 10) || 20, 1), 200);
        const skip = (page - 1) * limit;
        const sortBy = req.query.sortBy || 'createdAt';
        ...
        const rawPvIds = yield model_1.PointVente.distinct('_id', { region: regionOID });
        ...
        let pvMatchClause = {};
        if (req.query.pointVente && mongoose_1.default.isValidObjectId(String(req.query.pointVente))) {
        ...
        const execCount = includeTotal ? model_1.MouvementStock.countDocuments(finalFilter) : null;
        ...
        res.status((_c = err === null || err === void 0 ? void 0 : err.status) !== null && _c !== void 0 ? _c : 500).json({ message: (_d = err === null || err === void 0 ? void 0 : err.message) !== null && _d !== void 0 ? _d : 'Erreur interne' });
        return;
    }
});
