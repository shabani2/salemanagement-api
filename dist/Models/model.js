...
MouvementStockSchema.pre("save", function (next) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const { type, statut, produit, quantite, pointVente, region, depotCentral } = this;
            // Entrée & Commande: pas de contrôle de dispo
            if (type === "Entrée" || type === "Commande")
                return next();
            // LIVRAISON: vérifier le stock de la *source* uniquement
            if (type === "Livraison") {
                const { source, reasonIfInvalid } = (0, operationHandler_1.computeLivraisonScopes)(this);
                if (!source)
            ...
            if (["Vente", "Sortie"].includes(type)) {
                const { source, reasonIfInvalid } = (0, operationHandler_1.computeOperationSource)(this);
                if (!source)
...
MouvementStockSchema.post("save", function (doc) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const { produit, quantite, type, statut, pointVente, montant, region, depotCentral, transferApplied } = doc;
            // ENTRÉE: on crédite directement la destination (region sinon central)
            if (type === "Entrée") {
                if (region)
                    yield (0, operationHandler_1.adjustStock)({ produit, region }, quantite, montant);
                else
                    yield (0, operationHandler_1.adjustStock)({ produit, depotCentral: true }, quantite, montant);
                return;
            }
            ...
            if (["Vente", "Sortie"].includes(type)) {
                const { source, reasonIfInvalid } = (0, operationHandler_1.computeOperationSource)(doc);
                if (!source)
            ...
            // LIVRAISON:
            if (type === "Livraison") {
                const { source, destination, reasonIfInvalid } = (0, operationHandler_1.computeLivraisonScopes)(doc);
                if (!source)
                    return console.error(reasonIfInvalid);
                ...
                if (statut && destination && !transferApplied) {
                    yield (0, operationHandler_1.adjustStock)(destination, quantite, montant);
...
MouvementStockSchema.pre("findOneAndUpdate", function (next) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            this._oldDoc = yield this.model.findOne(this.getQuery()).lean().exec();
            next();
        }
        catch (e) {
            next(e);
...
MouvementStockSchema.post("findOneAndUpdate", function (resDoc) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const { destination, reasonIfInvalid } = (0, operationHandler_1.computeLivraisonScopes)(newDoc);
            if (!destination) {
